...
long read_entire_file(const char* filepath, unsigned char** buffer) {
  long length;
  FILE* file;
...
long load_font(const char* font_path, Font* font) {
  long length;
  ...
  for (int i = 0; i < FONT_NUM_LEVELS; i++) {
    ...
    // set texture image
    GL_CHECK(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, bw, bh, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                          colored_bitmap));
    free(bitmap);
    free(colored_bitmap);
...
void draw_text(const char* text, Font font, float line_height, float origin_x, float origin_y,
               float r, float g, float b, float a) {
  GLuint texture;
  norm_char_data_t* data;
  ...
  int i = 0;
  while (text[i] != '\0') {
    norm_char_data_t d = data[text[i] - 32];
...
GLuint load_shader(GLenum type, const char* shader_path) {
  GLuint shader;
  GLint compiled;
  GLint length;
...
GLuint load_texture(const char* image_path) {
  ...
  GL_CHECK(glPixelStorei(GL_UNPACK_ALIGNMENT, 1));
  GL_CHECK(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, image_width, image_height, 0, GL_RGBA,
...
GLuint load_shader_program(const char* vert_shader_path, const char* frag_shader_path) {
  ...
  vertex_shader = loadShader(GL_VERTEX_SHADER, vert_shader_path);
  if (vertex_shader == 0)
    return 0;

  fragment_shader = loadShader(GL_FRAGMENT_SHADER, frag_shader_path);
...
void draw_primitive(GLuint primitive, const Vec2 points[], int points_len, float r, float g,
                    float b, float a) {
...
void draw_line_loop(const Vec2 points[], int points_len, float r, float g, float b, float a) {
  draw_primitive(GL_LINE_LOOP, points, points_len, r, g, b, a);
}

void draw_points(const Vec2 points[], int points_len, float r, float g, float b, float a) {
  draw_primitive(GL_POINTS, points, points_len, r, g, b, a);
}

void draw_triangle_strip(const Vec2 points[], int points_len, float r, float g, float b, float a) {
  draw_primitive(GL_TRIANGLE_STRIP, points, points_len, r, g, b, a);
}

void draw_triangle_fan(const Vec2 points[], int points_len, float r, float g, float b, float a) {
  draw_primitive(GL_TRIANGLE_FAN, points, points_len, r, g, b, a);
}

void draw_triangles(const Vec2 points[], int points_len, float r, float g, float b, float a) {
  draw_primitive(GL_TRIANGLES, points, points_len, r, g, b, a);
}